...
class DQNAgent:
    ...
    def __init__(self, params):
        self.action_space_size = len(params['brake_action'])
        self._logfile = params['log_file']
        self._logfile.write("halo ini dari dqn")
        self.model_name = MODEL_NAME
        self.OBSERVATION_SPACE_DIMS = params['obs_shape']

        if(params['step_start'] > 0): 
            self.model = load_model("model/"+params['model_file'])
        else:
        ...
        self.tensorboard = params['tsb'](tf, log_dir=f"logs/{MODEL_NAME}")
